...
# ╔═╡ d54cb81d-b6b2-4f01-b9c2-49963c583b4a
using PlutoUI, Plots 
...
# ╔═╡ 6d076348-007f-4496-811c-4e71f1d6c79d
begin 
	
function Newton(f,f′,xo,n)
#Esta funcion utiliza la serie de Teylot para aproximar una 
#funcion. Consiste encontrar la raiz por medio de apoximaciones. 
#por medio de pendientes.
	#Error = 1000;
	#tol = 0.01
	i = 1;
	println("_._._._._._._._._._._._._._._._._._._._._._._._._._.\n")
	println("i\t     x0\t           x1\t   Error\t\n")
	println("_._._._._._._._._._._._._._._._._._._._._._._._._._.\n")
	x1 = 0
	...
	for i in 1:n 
		x1 = xo - (f.(xo)/f′.(xo))
		Error = abs(x1-xo)
		println("$i   $xo   $x1  $Error");
		xo = x1
		i = i + 1
	end 
	
	println("_._._._._._._._._._._._._._._._._._._._._._._._._._.\n");
	println("Resultado: $x1")
end
	f1(x) = x^3 +4*x^2 -10 
	f1′(x) = 3*x^2 +8*x
	x0 = 1
...
begin 
	x1 = 0
function newtonRecursiva(f,f′,xo,n)
	x1 = xo - (f.(xo)/f′.(xo))
	println("Resultado: $x1")

	for i in 1:n
		newtonRecursiva(f,f′,x1,n-i)
		println("Resultado: $x1")
		#Error = abs(x1-xo)
		#println("$i   $xo   $x1  $Error");
		#i = i + 1
...
function newtonDistancia(f,f′,xo,ε)
	i = 1                                                                                #Inicializamos i, contador el cual indicara el número de iteraciones 
	println("_._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._.\n")
	println("i\t          x0\t               x1\t               Error\t\n")
	println("_._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._._.\n")
	x1 = 0                                                                               #Inicializamos la variable x1 para que podamos imprimirla en pantalla posteriormente 
	Error = 1000                                                                       
...
derivadaNumérica(f, x, h) = h*(f.(x + (1/h)) - f.(x))
	f4(y)= sin(y)
	derivadaNumérica(f4, 1.0, 100000.0)
end 
...
#por medio de pendientes.
	...
	for i in 1:n 
		x1 = xo - (f.(xo)/derivadaNumérica(f, xo, 1000000000.0))
		Error = abs(x1-xo)
		println("$i   $xo   $x1  $Error");
		xo = x1
		i = i + 1
...
@bind X0 Slider(-10:0.1:10, show_value=true)



# ╔═╡ b214090e-53cc-4036-a281-1e5f341f7c78
@bind H Slider(1000:1000:1000000, show_value=true)

# ╔═╡ b16f9817-06ef-4234-9539-ce80a3599275
@bind N Slider(1:10000, show_value=true)

# ╔═╡ 9154fdd5-eba8-46ec-99e4-11d984bb228a
begin
function NewtonDN3(f,xo, h, n)
	i = 1;
	x1 = 0.0
	x = range(-5*pi, 5*pi, length = 1000)
    y = f.(x)
	plot(x, y, title="cos(x)", linewidth=3)

	for i in 1:n 
		x1 = xo - (f.(xo)/derivadaNumérica(f, xo, h))
		Error = abs(x1-xo)
		plot!([xo, x1], [f.(xo), f.(x1)], legend = false, color="black")
		xo = x1
		i = i + 1	
		scatter!([x1], [f.(x1)], legend = false)
	
	end 
		scatter!([x1], [f.(x1)], legend = false)
		hline!([0], color="black", label="", linewidth=1)
