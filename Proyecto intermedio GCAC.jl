...
# ╔═╡ bb3f5890-e2cc-11ed-2c34-4f6bf2903c48
using Images, ImageView, TestImages, PlutoUI, Colors, Plots


# ╔═╡ 207d0b10-24ae-4b6c-8b4c-dcaa4d4478f9
md"""## Aproximacion de pi

NOTA: No entiendo por qué dice que las variables PuntoCirculo y PuntosCuadros no están definidas. 
...
# ╔═╡ 3a9a79e1-972f-4e43-94b9-33bbecdbc583
#En este progrma calcula aproximaciones de pi por medio del medodo de Monte Carlo 
@bind Numero_de_Puntos Slider(1:10000, show_value=true)

# ╔═╡ 93fc70e0-6193-4e08-a7c5-36e73bac42b9
begin 

PuntosCirculo = 0                 #Inicializamos las variables 
PuntosCuadrado = 0
	

for i in 0:(Numero_de_Puntos)     #Definimos el número de iteraciones que necesitamos en el ciclo for para este caso es necesario el mismo número de puntos establecidos
	x = rand(Float64)       #Dentro del ciclo definimos dos variables x y y las                                cuales serán coordenadas  al azar del 0 al 1 
	y = rand(Float64)
	
	
	if (x-(1/2))^2 + (y-(1/2))^2 <= (1/2)^2        #Establecemos un condicional si se cumple que un punto (x,y) se encuentre dentro del círculo de radio R = 1/2 entonces dichos puntos se graficaran en azul y se sumara uno a los puntos dentro del círculo. 
		scatter!([x],[y], c=:blue, legend = false)
		PuntosCirculo += 1
	else 
		scatter!([x],[y], c=:pink, legend = false) #Los puntos que no cumplen lo anterior se graficaran en rosa y se sumara uno al contador de los puntos que estan fuera del circulo
		PuntosCuadrado += 1

		
	end 
	piAprox =(PuntosCirculo/Numero_de_Puntos)*4 #Finalmente aproximamos pi y el error
	error = ((piAprox - pi)/pi)
end 

# Imprimimos todos los datos
println("Puntos dentro del circulo: $PuntosCirculo")
println("Puntos fuera del circulo: $PuntosCuadrado")
println("Aproximacion a pi: $piAprox")
println("Numero total de puntos: $Numero_de_Puntos")
println("Error = $error")

end


# ╔═╡ 42386ebe-203d-4157-b3eb-da4137b5689f
begin 
## NOTA: Si se le aumenta cualquier cosa a este código deja de funcionar 
PuntosCirculo1 = 0


for i in 0:(Numero_de_Puntos)
	x = rand(Float64)
	y = rand(Float64)
	...
	pi =(PuntosCirculo1/Numero_de_Puntos)*4

end 
	
println("Puntos fuera del circulo $PuntosCirculo1")
println("Aprovimacion a pi: $pi")


end

# ╔═╡ 429a92e1-2125-45a6-8e97-d990b9f1eafe
md"""## Torres de Hanoi

Diseño del algoritmo 

Para resolver el problema de las Torres de hanoi se ideo un método de recursividad o sea que se repite un cierto número de veces. El algoritmo sería el siguiente partiendo de ciertos casos: 

N = Número de discos 

Si N = 0 entonces no se mueve nada.

Si N = 1 se mueve aquel disco de la primera a la última varilla 

Si N => 1 entonces se realiza lo siguientes:

 

1.     Mover la pila de discos que está sobre el disco más grande o sea los discos (N-1) a la varilla 1 a la segunda varilla o varilla de ayuda.

2.     Posteriormente mover el disco más grande o el número N de la varilla de inicio o sea la varilla 3 o a la varilla de destino.

3.     Posteriormente se mueve la pila de N – 1 discos de la varilla de ayuda a la varilla destino, esto considerando el mismo algoritmo ahora tomando en cuenta a la primera varilla como la de ayuda y a la segunda como la primera

 

Notese que para mover la pila de discos N – 1 es necesario realizar el mismo algoritmo tomando ahora el disco más grande como N – 1 y la pila de discos a mover como N -2, posteriormente la pila de N – 2 discos necesitará repetir el algoritmo y así sucesivamente hasta llegar a 1 disco. 

Enlace al diagrama de flujo: https://drive.google.com/file/d/1ZTA99N2LQ9QOkNfs1U46MfT4cXsPAaiP/view?usp=sharing


"""

# ╔═╡ 67aa9795-6de2-470e-ab06-2019250ed603
begin
println("TORRES DE HANOI")
println("Para resolver el problema de torres de Hanói ingresé en la función los siguientes parámetros: 
TorresHanoi(Número de discos, Torre 1, Torre 2, Torre 4) 
NOTA: los parámetros Torre 1, Torre 2 y Torre 3 solo acepta tipos de datos numéricos")

N =3                                     #Número de discos idealmente ingresado                                            por el usuario
N1 = ((2^N) -1)                      #Numero de movimientos 
	
println("\nNumero de movimientos: $N1 \n")

function TorresHanoi(N, V1, V2, V3)

	if N  == 1
		println("Mover disco $N de la Varilla $V1 a la Varilla $V3")

	else 
		TorresHanoi(N-1, V1,V3, V2)
		println("Mover disco $N de la Varilla $V1 a la Varilla $V3")
		TorresHanoi(N-1, V2, V1, V3)
	end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "e86f37a45f909b8399a069b594d61564ed68450b"
